...
const studentFields: DialogField[] = [
  ...
  { name: "classGrade", label: "Class / Grade", type: "select", required: true, options: ["Grade 10-A", "Grade 10-B", "Grade 11-A", "Grade 11-B", "Grade 12-A", "Grade 12-B", "Grade 12-C"] },
  { name: "guardianName", label: "Guardian / Parent Name", required: true },
...
const StudentListPage: React.FC = () => {
  ...
  const filtered = useMemo(() => students.filter((student) => `${student.name} ${student.email} ${student.rollNumber} ${student.classGrade}`.toLowerCase().includes(search.toLowerCase())), [students, search]);
  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const rows = filtered.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  function saveStudent(values: Record<string, string>) {
    if (editing) {
      setStudents((current) => current.map((student) => student.id === editing.id ? { ...student, ...values, enrollmentStatus: values.enrollmentStatus as Student["enrollmentStatus"], initials: values.name.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase() } : student));
    } else {
      const name = values.name.trim();
      const next: Student = { id: crypto.randomUUID(), sr: students.length + 1, name, email: values.email, initials: name.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase(), avatarColor: "bg-indigo-100 text-indigo-600", rollNumber: values.rollNumber, program: "General", classGrade: values.classGrade, guardianName: values.guardianName, contactNumber: values.contactNumber, dateJoined: values.dateJoined, attendanceDate: "Oct 24, 2024", arrivalTime: null, status: "Present", enrollmentStatus: values.enrollmentStatus as Student["enrollmentStatus"], attendanceRate: 100 };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        <table className="w-full min-w-[1080px] border-collapse text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((student, index) => <tr key={student.id} className="hover:bg-slate-50/60">
              ...
              <td className="px-4 py-3 text-slate-700">{student.rollNumber}</td><td className="px-4 py-3"><span className="rounded-full bg-indigo-50 px-2.5 py-1 text-[10px] font-medium text-indigo-700">{student.classGrade}</span></td><td className="px-4 py-3 text-slate-700">{student.guardianName}</td>
      ...
      <RecordDialog open={dialogOpen} entity="Student" initialValues={editing ? { name: editing.name, email: editing.email, rollNumber: editing.rollNumber, classGrade: editing.classGrade, guardianName: editing.guardianName, contactNumber: editing.contactNumber, dateJoined: editing.dateJoined, enrollmentStatus: editing.enrollmentStatus ?? "Active" } : undefined} fields={studentFields} onClose={() => setDialogOpen(false)} onSave={saveStudent} />
    </div>
  );
};
